...
# save_folder_path="tryon_checkpoints_with_densenet_correct_160"
save_folder_path="tryon_checkpoints_with_depth_background_correct"
...
def train():
    ...
    input_channels = 12
    model = networks.__dict__['Try_on_DenseNet'](device=device, inp_channels=input_channels)
    if args.use_cuda:
        print("Turn the model into CUDA")
        model = model.cuda()

    
    train_set = datasets.__dict__['VITON_bkgs'](train=True)
    val_set = datasets.__dict__['VITON_bkgs'](train=False)
    ...
    for t in range(251):
        ...
        # import sys
        # sys.exit()
        for i, (inp_1, inp_2, inp_3, inp_4) in enumerate(train_loader):
            diffs, res = model(inp_1.to(device), inp_2.to(device), inp_3.to(device), inp_4.to(device)) 
        ...
        for i, (inp_1, inp_2, inp_3, inp_4) in enumerate(val_loader):
            diffs, res = model(inp_1.to(device), inp_2.to(device), inp_3.to(device), inp_4.to(device)) 
            #  [negPSNR_loss(diff, args.epsilon) for diff in diffs]
            pixel_loss = [charbonier_loss(diff, args.epsilon) for diff in diffs]
            # total_loss = sum(x*y if x > 0 else 0 for x,y in zip(args.alpha, pixel_loss))
        ...
        if t % 10 == 0:
            print("Inside saving")
            save_file_name = os.path.join(save_folder_path, "tryon_epoch_res" + str(t+1) + "_" + str([round(training_losses.avg, 5)]) + "_" + str([round(val_losses.avg, 5)]) + ".pth")
